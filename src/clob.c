...
ssize_t level_pos(const struct level* level, oid_t order_id) {
    ...
    for (size_t i=0;i<level->num_orders;i++) if (level->orders[i].id == order_id) return i;
    return -1;
}
...
struct level* book_find_level_by_price(struct level** levels, size_t num_levels, uint64_t price) {
    if (levels == NULL || num_levels == 0) return NULL;
    for (size_t i=0;i<num_levels;i++) if (levels[i]->price == price) return levels[i];
    return NULL;
}
...
void book_delete_level(struct book* book, enum side side, size_t loff) {
    ...
    switch (side) {
        case BID: {
            ...
            free(to_delete);
            for (size_t i=loff;i<book->num_bid_levels-1;i++) book->bids[i] = book->bids[i+1];
            book->num_bid_levels--;
            break;
        }
        case ASK: {
            ...
            to_delete->num_orders =0;
            free(to_delete);
            for (size_t i=loff;i<book->num_ask_levels-1;i++) book->asks[i] = book->asks[i+1];
            book->num_ask_levels--;
            break;
        }
    }
...
void book_submit(struct book* book, struct order order) {
                        ...
                        if (curr_loff < book->num_ask_levels) {
                            ...
                            for (size_t i=0;i<curr_level->num_orders;i++) {
                                ...
                                if (curr_order->quantity > remaining_quantity) {
                                    ...
                                    return;
                                } else if (curr_order->quantity == remaining_quantity) {
                                    curr_order->quantity -= remaining_quantity;
                                    remaining_quantity = 0;
                                    level_remove_at(curr_level, i);
                                    if (curr_level->num_orders == 0) book_delete_level(book, ASK, curr_loff);
                                    return;
                                } else {
                                    ...
                                    if (curr_level->num_orders == 0) { 
                                        ...
                                        curr_loff--; /* adjust for later increment as we've just deleted this level */
                                        break;
                                    }
                                }
                            }
                            if (remaining_quantity > 0) curr_loff++;
                        } else goto post_order;
                    }
                } else goto post_order;
            } else goto post_order;
            break;
                        ...
                        if (curr_loff < book->num_bid_levels) {
                            ...
                            for (size_t i=0;i<curr_level->num_orders;i++) {
                                ...
                                if (curr_order->quantity > remaining_quantity) {
                                    ...
                                    remaining_quantity = 0;
                                    level_remove_at(curr_level, i);
                                    if (curr_level->num_orders == 0) book_delete_level(book, BID, curr_loff);
                                    return;
                                } else {
                                    remaining_quantity -= curr_order->quantity;
                                    curr_order->quantity = 0;
    ...
    /* no need to check for remaining as we handle that within the internal matching loop */
    return;
...
ssize_t book_new_level_offset(struct book* book, uint64_t price, enum side side) {
    ...
    switch (side) {
        case BID: {
            ...
            for (size_t i=0;i<book->num_bid_levels;i++) {
                ...
                if (curr_level->price == price || curr_level->price < price) return i;
            }
...
void book_insert(struct book* book, struct order order) {
    ...
    switch (order.side) {
        case BID: {
            struct level* level = book_find_level_by_price(book->bids, book->num_bid_levels, order.price);
            if (level == NULL) {
                size_t new_pos = book_new_level_offset(book, order.price, order.side);
                struct level** new_bids = realloc(book->bids, (book->num_bid_levels + 1) * sizeof(struct level*));
                if (new_bids == NULL) return;
                book->bids = new_bids;
                book->num_bid_levels++;
                for (size_t i=book->num_bid_levels-1;i>new_pos;i--) book->bids[i] = book->bids[i-1];
        ...
        case ASK: {
            struct level* level = book_find_level_by_price(book->asks, book->num_ask_levels, order.price);
            if (level == NULL) {
                size_t new_pos = book_new_level_offset(book, order.price, order.side);
                struct level** new_asks = realloc(book->asks, (book->num_ask_levels + 1) * sizeof(struct level*));
                if (new_asks == NULL) return;
                book->asks = new_asks;
                book->num_ask_levels++;
                for (size_t i=book->num_ask_levels-1;i>new_pos;i--) book->asks[i] = book->asks[i-1];
...
void book_free(struct book* book) {
    ...
    if (book->bids != NULL) { 
        for (size_t i=0;i<book->num_bid_levels;i++) level_free(book->bids[i]);
        free(book->bids);
    }

    if (book->asks != NULL) { 
        for (size_t i=0;i<book->num_ask_levels;i++) level_free(book->asks[i]);
        free(book->asks);
    }
    free(book);
}
...
void book_print_levels(struct level** levels, size_t num_levels) {
    ...
    for (size_t i=0;i<num_levels;i++) {
        ...
        for (size_t j=0;j<curr_level->num_orders;j++) printf("%ld ", curr_level->orders[j].quantity);
        printf("\n");
    }
}
